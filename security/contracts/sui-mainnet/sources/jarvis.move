...
/// Immutable genesis evidence used by deployment tooling and explorers.
/// It records the exact issuance recipient and monetary constants without
/// retaining any authority over the coin.
public struct GenesisReceipt has key, store {
    id: UID,
    recipient: address,
    whole_supply: u64,
    base_units: u64,
...
fun init(witness: JARVIS, ctx: &mut TxContext) {
    ...
    let genesis_receipt = GenesisReceipt {
        id: object::new(ctx),
        recipient,
        whole_supply: MAXIMUM_WHOLE_SUPPLY,
        base_units: MAXIMUM_BASE_UNITS,
        decimals: DECIMALS,
    };

    transfer::public_freeze_object(metadata);
    transfer::public_freeze_object(fixed_supply);
    transfer::public_freeze_object(genesis_receipt);
    transfer::public_transfer(complete_supply, recipient);
}
...
/// Returns the address that received the complete canonical genesis supply.
public fun genesis_recipient(receipt: &GenesisReceipt): address {
    receipt.recipient
}

/// Validates immutable genesis evidence against the compiled monetary policy.
public fun is_canonical_genesis(receipt: &GenesisReceipt): bool {
    receipt.whole_supply == MAXIMUM_WHOLE_SUPPLY &&
    receipt.base_units == MAXIMUM_BASE_UNITS &&
