...
export function canonicalSuiDeploymentReadiness(input: TokenDeploymentDescriptor): DeploymentReadinessReport {
  ...
  if (!input.sui?.genesisReceiptObjectId) reasons.push("sui-genesis-receipt-object-id-missing");
...
export function bridgedSolanaDeploymentReadiness(input: TokenDeploymentDescriptor): DeploymentReadinessReport {
  ...
  if (input.solana?.suiChainId !== 21) reasons.push("solana-sui-chain-domain-invalid");
  if (
    input.solana?.maxTransferBaseUnits === undefined ||
    input.solana.maxTransferBaseUnits <= 0n ||
    input.solana.maxTransferBaseUnits > JARVIS_TOKEN.maximumBaseUnits
  ) reasons.push("solana-transfer-limit-invalid");
...
export function deploymentCommitmentPayload(input: TokenDeploymentDescriptor): string {
  return canonicalJson({
    ...
    solana: input.solana ? {
      ...
      maxTransferBaseUnits: input.solana.maxTransferBaseUnits?.toString() ?? null,
    } : null,
